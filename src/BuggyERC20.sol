...
contract StableCoin is ERC20Burnable, Ownable(msg.sender) {
    ...
    function mint(address receiver, uint256 amount) public onlyOwner {
        //> only owner can mint so the user can't mint another one for themselves
        _mint(receiver, amount);
    }

    function burn(address from, uint256 amount) public {
        //> @audit anyone can burn someone else's token
        _burn(from, amount);
    }
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        // if a user is frozen,
        // he can approve someone else to spend their tokens
        // it will work as long as the spender is not frozen
